...
export const appendStream = (chunk, scrollCallback) => {
    ...
    while (i < newContent.length) {
        ...
        if (state.inThink) {
            ...
            // Remove streaming class and collapse when think block completes
            if (state.thinkAccEl) {
                state.thinkAccEl.classList.remove('streaming');
                state.thinkAccEl.open = false;
                const summary = state.thinkAccEl.querySelector('summary');
                if (summary && summary.textContent.startsWith('Reasoning')) {
                    summary.textContent = 'Reasoning';
                    // Capture button — injects reasoning as a quoted user note into chat history
                    const capturedThink = state.thinkBuf;
                    const captureBtn = createElem('button', { class: 'think-capture-btn', title: 'Inject reasoning into chat context' }, '→ chat');
                    captureBtn.addEventListener('click', async (e) => {
                        e.stopPropagation();
                        captureBtn.disabled = true;
                        captureBtn.textContent = '✓';
                        try {
                            const { fetchRawHistory, importChat } = await import('./api.js');
                            const { refresh } = await import('./core/state.js');
                            const history = await fetchRawHistory();
                            const messages = Array.isArray(history) ? history : (history.messages || []);
                            messages.push({
                                role: 'user',
                                content: `Here's your chain of reasoning revealed to you:\n\n${capturedThink.trim()}`
                            });
                            await importChat(messages);
                            await refresh(false);
                        } catch (err) {
                            console.error('[think-capture] failed:', err);
                            captureBtn.textContent = '!';
                        }
                    });
                    summary.appendChild(captureBtn);
                }
            }
...
export const startTool = (toolId, toolName, args, scrollCallback) => {
    ...
    document.dispatchEvent(new CustomEvent('terminus:tool_start', {
