...
const VIEW_MODULES = {
    ...
    watchdogs: `./views/watchdogs.js${_v}`,
...
function initAppearance() {
    ...
    // Density
    const density = localStorage.getItem('terminus-density');
    if (density && density !== 'default') {
        root.setAttribute('data-density', density);
    }

    // Font
    const font = localStorage.getItem('terminus-font');
    if (font && font !== 'system') {
        root.setAttribute('data-font', font);
    }

    // Clean up stale trim localStorage (now per-persona)
    localStorage.removeItem('terminus-trim');

    // Send button trim preference
    const sendBtnTrim = localStorage.getItem('terminus-send-btn-trim');
    if (sendBtnTrim === 'true') {
...
function initEventBus() {
    ...
    eventBus.on(eventBus.Events.TTS_SPEAK, (data) => {
        ...
        const claimKey = 'terminus_tts_claim';
    ...
    document.addEventListener('terminus:plugin_toggled', () => reloadPluginScripts());
