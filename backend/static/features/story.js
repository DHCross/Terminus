...
export async function openStoryPicker() {
    ...
    // Reflections should surface user-owned content, not built-in Sapphire stories.
    let reflectionPresets = [];
    try {
        const data = await fetchWithTimeout('/api/story/presets?source=user');
        reflectionPresets = (data.presets || []).filter(p => !p.name.startsWith('_'));
    } catch (e) {
        console.warn('Failed to load reflection presets:', e);
    }

    const grid = modal.querySelector('#story-preset-grid');
    if (grid) {
        if (!reflectionPresets.length) {
            grid.innerHTML = `
                <div class="story-preset-empty">
                    <span class="story-preset-card-title">No reflections yet</span>
                    <span class="story-preset-card-desc">Your own reflection presets will appear here when added under user/story_presets.</span>
                </div>
            `;
        } else {
            grid.innerHTML = reflectionPresets.map(p => {
            const tags = [];
