...
import { avatarUrl, listPersonas } from '../shared/persona-api.js';
...
async function openModal() {
    try {
        const [data, personasResponse] = await Promise.all([
            getInitData(),
        ...
        // Persona list
        const personas = personasResponse?.personas || data.personas?.list || [];
        const defaultP = personasResponse?.default || data.personas?.default || '';
