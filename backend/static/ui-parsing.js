...
// Renders <ipython>...</ipython> blocks as a collapsible action indicator card.
// Shows function names as chips; expands to show full call signatures on click.
const createIpythonCard = (blocks) => {
    const card = createElem('div', { class: 'action-card' });

    const allCalls = [];
    blocks.forEach(block => {
        block.split('\n').map(l => l.trim()).filter(l => l && !l.startsWith('#')).forEach(line => {
            const fnMatch = line.match(/^(\w+)\s*\(/);
            allCalls.push({ name: fnMatch ? fnMatch[1] : line, raw: line });
        });
    });
    if (allCalls.length === 0) return card;

    const header = createElem('div', { class: 'action-card-header' });
    const icon = createElem('span', {}, '⚙\uFE0F');
    const label = createElem('span', { class: 'action-card-label' },
        `${allCalls.length} action${allCalls.length !== 1 ? 's' : ''}`);
    const chips = createElem('span', { class: 'action-card-chips' });
    allCalls.forEach(({ name }) => {
        chips.appendChild(createElem('span', { class: 'action-chip-pill' }, name));
    });
    const arrow = createElem('span', { class: 'action-card-arrow' }, '▸');
    ...
    const body = createElem('div', { class: 'action-card-body' });
    body.style.display = 'none';
    allCalls.forEach(({ raw }) => {
        body.appendChild(createElem('div', { class: 'action-chip-full' }, raw));
    });
...
const processInlineMarkdown = (html) => {
    ...
    html = html.replace(/!\[([^\]]*)\]\s*\(([^()]*(?:\([^)]*\)[^()]*)*)\)/g, (m, alt, url) => {
        ...
        if (/^https?:\/\//i.test(url)) return `<img src="${url}" alt="${alt}" class="chat-img" loading="lazy" decoding="async">`;
        return m;
    });
...
export const wrapImageGalleries = (container) => {
    ...
    container.querySelectorAll('.chat-img').forEach(img => {
        ...
        console.error('[wrapImageGalleries] failed:', e);
    }
};
...
const renderContentText = (el, txt, isHistoryRender, scrollCallback, thinkCnt) => {
    ...
    // Handle <ipython> action blocks — strip from prose and render as indicator card
    if (txt.includes('<ipython>')) {
        const ipyBlocks = [];
        txt = txt.replace(/<ipython>([\s\S]*?)<\/ipython>/gi, (_, inner) => {
            ipyBlocks.push(inner.trim());
            return '';
        }).trim();
        if (ipyBlocks.length > 0) {
            el.appendChild(createIpythonCard(ipyBlocks));
        }
    }
