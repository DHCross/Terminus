  ...
  async finish() {
    ...
    if (needsRestart) {
      ...
      this.modal.querySelector('.setup-wizard-footer').innerHTML = `
        ...
        <button class="btn btn-primary" id="setup-restart">Restart Terminus</button>
      `;
  ...
  async triggerRestart() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert('Failed to restart. Please restart Terminus manually.');
        this.close();
      }
    } catch (e) {
      ...
      content.innerHTML = `
        <div class="success-screen">
          ...
          <p class="provider-loading-hint">
            ...
            Models still downloading — Terminus will activate them automatically when ready.
          </p>
        </div>
      `;
    } else {
