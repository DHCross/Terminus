// views/spices.js - Style manager view with style sets
...
const DEFAULT_ICONS = {
    default: '\u{1F916}', companion: '\u{1F49C}', professional: '\u{1F4BC}',
        ...
        <div class="two-panel">
            <div class="panel-left panel-list">
                <div class="panel-list-header">
                    <span class="panel-list-title">Style Sets</span>
            ...
            <div class="panel-right">
                ...
                <div class="view-body view-scroll">
                    <div class="ss-actions-bar">
                        ...
                        <button class="btn-sm" id="spice-reload" title="Reload style pool from disk">Reload Pool</button>
                    </div>
                ...
                <div class="spice-cat-body">
                    <div class="spice-cat-inner">
                        <div class="spice-cat-actions">
                            <button class="btn-sm" data-action="add-spice" data-cat="${name}">+ Style</button>
                        ...
                        ${spices.length === 0 ? '<div class="text-muted" style="padding:8px;font-size:var(--font-sm)">Empty \u2014 add a style above</div>' :
                        spices.map((text, i) => `
...
function bindEvents() {
    ...
    container.querySelector('#ss-new')?.addEventListener('click', async () => {
        const name = prompt('New style set name:');
        if (!name?.trim()) return;
    ...
    container.querySelector('#ss-delete')?.addEventListener('click', async () => {
        if (!confirm(`Delete style set "${selectedSetName}"?`)) return;
        try {
...
async function handleSpiceAction(e) {
    ...
    try {
        if (action === 'add-spice') {
            const text = prompt(`Add style to "${cat}":`);
            if (!text?.trim()) return;
            ...
            const text = prompt('Edit style:', current);
