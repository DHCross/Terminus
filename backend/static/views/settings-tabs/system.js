...
export default {
    ...
    render(ctx) {
        return `
            ...
            <div class="system-tools" style="margin:20px 0;padding:16px;border:1px solid var(--border);border-radius:var(--radius)">
                ...
                <p class="text-muted" style="font-size:var(--font-xs);margin:4px 0 12px">Configure LLM, audio, voice, and identity settings step by step.</p>
                <button class="btn-primary" id="sys-restart">Restart Terminus</button>
                <p class="text-muted" style="font-size:var(--font-xs);margin:4px 0 0">Gracefully restarts the backend and reloads the UI when ready.</p>
            </div>
    ...
    attachListeners(ctx, el) {
        el.querySelector('#sys-restart')?.addEventListener('click', async () => {
            if (!confirm('Restart Terminus backend?')) return;
            const btn = el.querySelector('#sys-restart');
            btn.disabled = true;
            ...
            // Poll until the backend is back up, then reload
