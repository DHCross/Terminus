    ...
    container.innerHTML = `
        <div class="mind-view">
            <div class="mind-header">
                <h2>Mind</h2>
                <div class="mind-tabs">
                    <button class="mind-tab${activeTab === 'memories' ? ' active' : ''}" data-tab="memories">Memory</button>
                    <button class="mind-tab${activeTab === 'people' ? ' active' : ''}" data-tab="people">People</button>
                    <button class="mind-tab${activeTab === 'knowledge' ? ' active' : ''}" data-tab="knowledge">Sources</button>
...
async function renderMemories(el) {
    ...
    const desc = '<div class="mind-tab-desc">Durable continuity claims grouped by label: project state, decisions, hypotheses, contradictions, preferences, and self-audits.</div>';
    ...
    el.querySelectorAll('.mind-edit-memory').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            const memory = Object.values(groups).flat().find(m => m.id === id);
            showMemoryEditModal(el, id, content, memory?.label || '');
        });
    });
...
function showMemoryEditModal(el, memoryId, content, label = '') {
    const existing = document.querySelector('.mind-modal-overlay');
    if (existing) existing.remove();

    const labelOptions = [
        'project', 'preference', 'decision', 'hypothesis', 'contradiction',
        'evidence', 'source', 'person', 'workflow', 'open-question',
    ...
    overlay.innerHTML = `
        <div class="pr-modal">
            ...
            <div class="pr-modal-body">
                <div class="mind-form">
                    <input type="text" id="mm-label" placeholder="Label" list="mm-label-options" value="${escAttr(label)}">
                    <datalist id="mm-label-options">
                        ${labelOptions.map(option => `<option value="${escAttr(option)}"></option>`).join('')}
                    </datalist>
    ...
    overlay.querySelector('#mm-save').addEventListener('click', async () => {
        ...
        const newLabel = overlay.querySelector('#mm-label')?.value.trim().toLowerCase() || null;
        if (!newContent) { ui.showToast('Content cannot be empty', 'error'); return; }
        if (newContent === content && (newLabel || '') === (label || '')) { close(); return; }
        try {
            const resp = await fetch(`/api/memory/${memoryId}`, {
                method: 'PUT',
                headers: csrfHeaders({ 'Content-Type': 'application/json' }),
                body: JSON.stringify({ content: newContent, label: newLabel, scope: currentScope })
            });
            if (resp.ok) {
                close();
...
async function renderPeople(el) {
    ...
    el.innerHTML = `
        <div class="mind-tab-desc">People and contact context available to the active scope.</div>
        <div class="mind-toolbar">
            <button class="mind-btn" id="mind-add-person">+ Add Person</button>
            <button class="mind-btn" id="mind-import-google-csv">Import Google CSV</button>
            <button class="mind-btn" id="mind-import-vcf">Import VCF</button>
            <input type="file" id="mind-google-csv-input" accept=".csv,text/csv" style="display:none">
    ...
    const csvInput = el.querySelector('#mind-google-csv-input');
    el.querySelector('#mind-import-google-csv')?.addEventListener('click', () => csvInput?.click());
    csvInput?.addEventListener('change', async (e) => {
        ...
        try {
            const resp = await fetch('/api/knowledge/people/import-google-csv', { method: 'POST', headers: csrfHeaders(), body: form });
            if (!resp.ok) {
                const err = await resp.json().catch(() => ({}));
                throw new Error(err.detail || 'Upload failed');
            }
            const result = await resp.json();
            let msg = `Imported ${result.imported} and updated ${result.updated || 0} of ${result.total_in_file} contacts`;
            ...
            ui.showToast(msg, (result.imported || result.updated) ? 'success' : 'info');
            await renderPeople(el);
        } catch (err) { ui.showToast('CSV import failed: ' + err.message, 'error'); }
        csvInput.value = '';
    });
...
async function renderKnowledge(el, tabType) {
    ...
    const knDesc = isAI
        ? 'AI-authored research notes and self-maintenance records.'
        : 'Reference sources and notes searchable by the active scope.';
