...
function preserveExistingConfig(baseConfig, existingConfig, repoRoot) {
  ...
  if (Array.isArray(existingConfig.paths?.source_roots) && existingConfig.paths.source_roots.length > 0) {
    const valid = repoRoot 
      ? existingConfig.paths.source_roots.filter(rel => isDirectory(path.join(repoRoot, rel)))
      : existingConfig.paths.source_roots;
    if (valid.length > 0) {
      merged.paths.source_roots = valid;
    }
  }
  if (Array.isArray(existingConfig.paths?.test_roots) && existingConfig.paths.test_roots.length > 0) {
    const valid = repoRoot 
      ? existingConfig.paths.test_roots.filter(rel => isDirectory(path.join(repoRoot, rel)))
      : existingConfig.paths.test_roots;
    if (valid.length > 0) {
      merged.paths.test_roots = valid;
    }
  }
...
function main() {
  ...
  const config = preserveExistingConfig(generatedConfig, existingConfig, repoRoot);
