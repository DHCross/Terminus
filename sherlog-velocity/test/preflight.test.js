...
const { runPreflight, summarizeTracePacket, parseArgs } = require('../src/cli/preflight');
...
describe('preflight parseArgs', () => {
  ...
  test('parses --trace flag with fast mode enabled by default', () => {
    const args = parseArgs(['node', 'preflight.js', '--trace', '--feature', 'Trace Feature']);
    assert.equal(args.trace, true);
    assert.equal(args.fast, true);
    assert.equal(args.feature, 'Trace Feature');
  });

  test('parses start alias as trace mode', () => {
    const args = parseArgs(['node', 'preflight.js', 'start', '--feature', 'Trace Feature', '--no-fast']);
    assert.equal(args.trace, true);
    assert.equal(args.fast, false);
  });
});

describe('trace preflight summary', () => {
  test('prioritizes verification failures as the one thing to fix', () => {
    const result = summarizeTracePacket({
      verify: {
        checks: [{ id: 'config', status: 'fail', message: 'Config missing' }],
      },
      doctor: {},
      gaps: { gaps: ['arch_monolith', 'test_coverage'] },
    });

    assert.equal(result.status, 'blocked');
    assert.equal(result.hidden_low_priority_gaps, 1);
    assert.deepEqual(result.high_priority_gaps, ['test_coverage']);
    assert.match(result.one_thing, /Verification Failure: config/);
  });

  test('hides low-priority architecture gaps when no critical hazard exists', () => {
    const result = summarizeTracePacket({
      verify: { checks: [] },
      doctor: {},
      gaps: { gaps: ['arch_monolith', 'architectural_limit_exceeded', 'missing_bundle'] },
    });

    assert.equal(result.status, 'caution');
    assert.equal(result.hidden_low_priority_gaps, 2);
    assert.deepEqual(result.high_priority_gaps, ['missing_bundle']);
    assert.equal(result.one_thing, 'Resolve gap: missing_bundle');
  });
});
...
describe('preflight CLI', () => {
  ...
  test('emits trace JSON with --trace --json', () => {
    const repoRoot = makeRepoRoot('cli-trace-json');
    fs.writeFileSync(
      path.join(repoRoot, 'package.json'),
      JSON.stringify({
        scripts: {
          'sherlog:verify': 'node sherlog-velocity/src/cli/verify.js',
    ...
    const scriptPath = path.join(__dirname, '..', 'src', 'cli', 'preflight.js');
    const raw = run(`node "${scriptPath}" --trace --feature "Trace CLI" --json`, repoRoot);
    const payload = JSON.parse(raw);

    assert.equal(payload.schema_version, 'sherlog.trace_preflight.v1');
    assert.equal(payload.mode, 'recursive_trace_reinjection');
    assert.equal(payload.feature, 'Trace CLI');
