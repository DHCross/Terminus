...
function checkBranchInflation(repoRoot) {
  const { execSync } = require('child_process');
  const shared = require('../core/shared');
  const info = shared.detectBranchHead ? shared.detectBranchHead(repoRoot) : null;

  if (!info || info.branch === 'unknown' || info.branch === 'main' || info.branch === 'master') {
    return null;
  }
  ...
  try {
    ...
    const hasBase = runGit(`git branch --list ${baseBranch}`);
    if (!hasBase) return null;

    const commitCountLine = runGit(`git rev-list --count ${baseBranch}..${info.branch}`);
    if (!commitCountLine) return null;
    const commitCount = parseInt(commitCountLine, 10);

    const diffStat = runGit(`git diff --shortstat ${baseBranch}..${info.branch}`);
    ...
    const thresholdCommits = 12;
    const thresholdFiles = 25;
    const thresholdInsertions = 600;

    const isInflated = commitCount > thresholdCommits || filesChanged > thresholdFiles || insertions > thresholdInsertions;

    if (isInflated) {
      return {
        id: 'branch_inflation',
        status: 'warn',
        message: `Branch "${info.branch}" is inflating (${commitCount} commits, ${filesChanged} files, +${insertions} lines) and has not been integrated into ${baseBranch}.`,
        fix: `Consider rebasing and merging "${info.branch}" to ${baseBranch} to reduce deployment drift.`,
        evidence: {
          branch: info.branch,
          base: baseBranch,
          commits: commitCount,
    ...
    return {
      id: 'branch_inflation',
      status: 'pass',
      message: `Branch "${info.branch}" integration drift is within safe bounds.`,
      evidence: {
        branch: info.branch,
        base: baseBranch,
        commits: commitCount,
