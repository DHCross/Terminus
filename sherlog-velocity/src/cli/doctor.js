...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (arg === '--skip-tests') out.skip_tests = true;
    else if (arg === '--skip-hygiene') out.skip_hygiene = true;
    else if (!arg.startsWith('-')) out.feature = out.feature ? `${out.feature} ${arg}` : arg;
  }

  if (out.fast) {
    out.skip_tests = true;
    out.skip_hygiene = true;
  }
...
function main() {
  ...
  const hygieneSkipped = args.skip_hygiene || args.skip_tests;
  ...
  const output = {
    ...
    hygiene: hygieneResult ? {
      ...
      skipped: Boolean(hygieneSkipped),
      reason: hygieneSkipped ? 'Skipped hygiene scan in fast mode' : 'Unavailable',
    },
    recommendation,
    commands: {
      doctor: `npm run sherlog:doctor -- --feature "${feature}" --json`,
      doctor_fast: `npm run sherlog:doctor:fast -- --feature "${feature}" --json`,
