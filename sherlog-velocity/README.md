...
| `task preflight -- ...` | `npm run sherlog:preflight` | **Preferred AI-agent entry point.** Runs unified recursive trace preflight for `verify`, fast `doctor`, `gaps`, and `prompt`. |
...
**`npm run sherlog:preflight`** (or `task preflight`) is the preferred entry point for AI agents before making code mutations. It runs recursive trace preflight: `verify`, fast `doctor`, `gaps`, and `prompt` are reconciled into a single packet with one recommended next action.

The lower-level telemetry mode still exists for direct blast-radius, lint-plan, and bounds checks. Call it directly with `node sherlog-velocity/src/cli/preflight.js --file ...` or `--plan-file ...` when you need those specific instruments.

### Usage

```bash
# Unified trace packet for a feature
npm run sherlog:preflight -- --feature "Add user authentication" --json

# Analyze a file's blast radius with lower-level telemetry mode
node sherlog-velocity/src/cli/preflight.js --file src/app/api/raven-chat/protocolRules.ts --json

# Validate a plan file with lower-level telemetry mode
node sherlog-velocity/src/cli/preflight.js --plan-file plan.json --json

# Get bounds for a feature with lower-level telemetry mode
node sherlog-velocity/src/cli/preflight.js --feature "Add user authentication" --json
```
...
```json
{
  ...
  "summary": {
    ...
    "one_thing": "single recommended fix or null",
    "high_priority_gaps": [],
    "hidden_low_priority_gaps": 0
  }
}
```

Lower-level telemetry schema:
...
Trace preflight orchestrates these commands:

- `verify` for install and repo-contract wiring
- `doctor --fast` for health and recommendation without long-running scans
- `gaps --summary-only` for signal-ranked gap evidence
- `prompt` for the agent handoff payload

Lower-level telemetry preflight composes these instruments:
